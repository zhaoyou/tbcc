...
<script type="text/javascript">
		...
		function goEditStore(){	
				var $chkStoreType = $('form input:checkbox') ;
				if(!$chkStoreType.length){
					window.alert('没有可选的存储类型');
					return  ;
				}
				var flag = false ;
				if(!$chkStoreType.filter(":checked").length){
					window.alert('请选择一个编辑的存储类型');
					return  ;
				}
				
				if($chkStoreType.filter(":checked").length>1){
					window.alert("一次只允许编辑一个存储类型");
					return ;
				}
		...
		function goDelStore(){
			...
			if(!$chkStoreType.filter(":checked").length){
				window.alert('请选择需要删除的存储类型');
				return  ;
			}
		
			if(window.confirm("所选存储类型对应的冷库、探头也会删除掉！你确定 ？")){
				$('#ope').val("toDelStore");
				$("#myform")[0].submit();
			}
		}
...
<form action="storeconfig.do" id="myform" name="myform" method="post">
<input type="hidden" value="" name="ope" id="ope"/>
<div id="content">
  <div id="center" >
    <div class="top" style="display:block;">
      <table id="tb" border="0" cellspacing="0" cellpadding="0" >
        <tr id="tb1" >
          <td colspan="7"><span style="text-align:left; float:left;padding-left:4px; margin-left:4px; color:#141414;"><img src=../images/admin/index/icon.gif width="9" height="10" /> 你当前所在位置为：冷库的仓储类型配置</span></td>
        </tr>
      </table>
    </div>
    <div class="bottom">
      <table id="tb" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr class="altrow" >
          <td  rowspan="2" class="altrow_d" >选择 </td>
          <td rowspan="2" class="altrow_d" >仓储类型名称</td>
          <td colspan="4">温度</td>
        ...
        <tbody>
        	<c:forEach var="store" items="${storeTypeList}" varStatus="irow">
        			<tr style="height:28px;" ${irow.index%2==0?"":"bgcolor='#f1f4f8'"}>
			          <td><input name="chkStoreType" type="checkbox" id="chkStoreType" value="${store.storeid }" /></td>
			          <td >${store.name }</td>
			          <td >${store.templowerlimit  }</td>
			          <td >${store.templowerdelay }</td>
			          <%-- <td >${store.tempminlowerlimit  }</td>
			          <td  >${store.tempminlowerdelay }</td>--%>
			          <td >${store.temphighlimit  }</td>
			          <td >${store.temphighdelay }</td>
			         <%--  <td >${store.tempmaxhighlimit  }</td>
			          <td >${store.tempmaxhighdelay }</td>--%>
			          <td >${store.humlowerlimit  }</td>
			          <td >${store.humlowerdelay  }</td>
			          <td>${store.humhighlimit }</td>
			          <td >${store.humhighdelay }</td>
			        </tr>
        	</c:forEach>
        </tbody>
        <tr>
          <td colspan="14"><br />
            ...
            <table width="64%" class="oii">
              <tr>
                <td><a href="javascript:goAddStore();" target="_parent"><img src="../images/admin/index/icon_f.gif" width="86" height="23"  style="cursor:pointer;"/></a></td>
                <td><a href="javascript:goEditStore();"><img src="../images/admin/index/icon_a.gif" width="86" height="23" /></a></td>
                <td><a href="javascript:goDelStore();"><img src="../images/admin/index/icon_b.gif" width="86" height="23" /></a></td>
              </tr>
            </table>
            <br /></td>
        </tr>
