   ...
   */
    ...
    function printData(time1,time2){
        ...
        // 获取表单数据对象
       	...
       	//获取行列的数量
   	   var hang = table.rows.length-5;			//-5，是为了减去后面的4行统计信息，以及一个横杠
	   var lie = table.rows(0).cells.length+1;  //+1，是为了增加前面那个序号
	   
	  var flag = true ; 
	   if(hang>150){
	      flag =  window.confirm("您打印的数量太大,将花费30(S)左右.你确定吗?")  ;   
	  	 if(!flag)
	       return ;
	   }else{
	   	window.alert("您打印这些数据,将花费15(S)左右.");
	   }
      ...
      //  var project = document.all.proId ;
	   ...
	   objxls.Columns(1).ColumnWidth = 4 ;
  		...
  		//由于取消了gps信息，所以这里不需要设置宽度
        //objxls.Columns(3+Tcount+Hcount).ColumnWidth = 10 ;
        // objxls.Columns(3+Tcount+Hcount+1).ColumnWidth = 10 ;
        
        //设置温湿度的最大值、最小值、平均值的列宽
        /**
        **这里循环需要注意，Columns(5+0+Tcount+Hcount) 5变成了3 现在只有2列了(序号和时间)，去掉了gps
        */
        for(var o = 0 ;o<7 ;o++)
        	objxls.Columns(3+o+Tcount+Hcount).ColumnWidth = 6.5 ;
        
        //设置温湿度列头的宽度
        for(var i=0;i<Tcount+Hcount;i++){
            objxls.Columns(3+i).ColumnWidth = 5
        }
	     ...
	     objxls.Columns(i).HorizontalAlignment = 3 ;
            ...
            if(i==1)
                objxls.Cells(1,i).value = "编号" ;
                 ...
                 //设置时间字段的宽度为19.13
                  if(i==2){ 
                    objxls.Cells(1,i).ColumnWidth = 19.15 ;
                  }
