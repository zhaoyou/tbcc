...
<title>菜单组操作</title>
<link href="../css/admin/index/typeAdd.css" rel="stylesheet" type="text/css" />
...
<script type="text/javascript">
		...
		function goAdd(url){
			//验证是否有必须输入项为空
			if($('#ltname').val()==""){
				$('#ltname').css('borderColor','red');
				$('#ltnameTip').css('color','red');
				$('#tip').html( "请确保信息完整!" ) ;
				return ;
			}else{
				$('#ltname').css('borderColor','gray');
				$('#ltnameTip').css('color','black');
			}
			
			if($('#ltname').val().length>25){
				$('#ltname').css('borderColor','red');
    ...
    <div class="top" style="display:block;">
      <table id="tb" border="0" cellspacing="0" cellpadding="0" >
        <tr id="tb1" >
          ...
          你当前所在位置为：菜单组配置-><c:if test="${linktype==null}">增加菜单组</c:if><c:if test="${linktype!=null}">修改菜单组</c:if></span></td>
        </tr>
      </table>
    </div>
    <div class="bottom">
      <div class="con_center">
        <form id="myform" name="myform" action="" method="post">
			...
			<input type="hidden" name="typeid" id="typeid" value="${linktype.ltid }" />
			
			<p style="padding-top: 0px;padding-bottom: 8px;">&nbsp;</p>
          
          <p>
          	<label style="width: 60px;"></label>
            <label for="fdsafa" style="width:75px;">菜单组名称：</label>
            <input name="ltname" id="ltname" type="text" value="${linktype!=null?linktype.ltname:'' }" class="input_a" style="width: 150px;"/>
            <span id="ltnameTip" style="text-align:left; margin-left:5px;font-weight: bold;">*</span>
          </p>
          
          <p style="padding-top: 2px;padding-right: 0px;padding-bottom:0px;padding-left: 0px; display:block;"><span style="float: none;"></span></p>
            ...
            <input type="button" value="保存" class="but_a"  onclick="javascript:goAdd('linktype.do');"/>
            <input type="button" value="取消" class="but_b" onclick="javascript:gocancel('linktype.do','toLinktype');"/>
          </p>
        </form>
      </div>
    </div>
