...
<title>菜单组列表</title>
<link href="../css/admin/index/typeList.css" rel="stylesheet" type="text/css" />
...
<script type="text/javascript">
		...
		function godelete(url,ope){
			...
			if(chkId.length>0){
				...
				if(idStr!=""){
					if(confirm("确定要删除该菜单组以及该连接组下的所有菜单？")){
		...
		function goupdate(url,ope){
			...
			if(chkId.length>0){
				...
				if(count>1){
					alert("只可以选择一个菜单组进行修改");
					return;
				}
				if(count==1){
					document.getElementById("typeid").value = upId;
					//alert(upId);
					goaction(url,ope);
				}
			}
		...
		function goLinkConfig(ltid){
			document.getElementById("typeid").value=ltid;
			goaction("linkconfig.do","toLinkConfig");
		}
  ...
  <div id="center" >
    ...
    <div class="bottom">
      <table id="tb" border="0" cellspacing="0" cellpadding="0">
        <tr class="altrow" >
          <td width="80">选择</td>
          <td width="380">菜单组名称</td>
          <td width="450">操作</td>
        </tr>
        <tbody>
        	<logic:notEmpty name="typeList">
	        	<c:forEach var="type" items="${typeList}" varStatus="irow">
	        		<tr style="height:25px;" ${irow.index%2==0?"":"bgcolor='#f1f4f8'"}>
	        		  <td><input type="checkbox" name="chkId" id="chkId" value="${type.ltid }" /></td>
			          <td><span>${type.ltname }</span></td>
			          <td><a href="javascript:goLinkConfig(${type.ltid });">配置菜单组</a></td>
			        </tr>
	        	</c:forEach>
        	</logic:notEmpty>
        </tbody>
        ...
        <tr>
          <td colspan="3"><div id="tip">${tip }<br /></div>
            <table width="64%" class="oii">
              <tr>
                <td><a href="javascript:goAdd('linktype.do','toTypeAdd');" target="_parent"><img src="../images/admin/index/icon_f.gif" width="86" height="23"  style="cursor:pointer;"/></a></td>
                <td><a href="javascript:goupdate('linktype.do','toTypeUp');"><img src="../images/admin/index/icon_a.gif" width="86" height="23" style="cursor:pointer;" /></a></td>
                <td><a href="javascript:godelete('linktype.do','doTypeDel');"><img src="../images/admin/index/icon_b.gif" width="86" height="23" style="cursor:pointer;" /></a></td>
              </tr>
            </table>
